function lex#fuzzy#start(fn)
    let wrap = fzf#wrap({'source': luaeval("require'lex.link'.Location.list.all()")})
...
function lex#fuzzy#goto(lines)
    call luaeval("require'lex.link'.fuzzy.sink.goto(_A)", a:lines)
endfunction

function lex#fuzzy#put(lines)
    call luaeval("require'lex.link'.fuzzy.sink.put(_A)", a:lines)
endfunction

function lex#fuzzy#insert_put(lines)
    call luaeval("require'lex.link'.fuzzy.sink.insert_put(_A)", a:lines)
endfunction
