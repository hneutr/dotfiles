...
describe("Mirror", function()
    ...
    describe("init", function()
        it("infers path", function()
            eq(Mirror().path, Path.current_file())
        end)
    end)
    ...
    describe("get_type", function()
        it("non-mirror", function()
            ...
            eq(Mirror.get_type(_path('file.md'), config.get()), "origin")
        end)
    
        it("mirror", function()
            Projector.set_mirrors({x = {kind='a'}})
            eq(Mirror.get_type(_path('a/x/file.md'), config.get()), "x")
        end)

        it("nested mirror", function()
            Projector.set_mirrors({x = {kind='a'}, y = {kind='a'}})
            eq(Mirror.get_type(_path('a/x/y/file.md'), config.get()), "x")
        end)
    end)
